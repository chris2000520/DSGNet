...
def get_model(config):
    model_hub = {'adscnet':ADSCNet, 'aglnet':AGLNet, 'bisenetv1':BiSeNetv1, 
                ...
                'stdc':STDC, 'swiftnet':SwiftNet, 'cgfnet': CGFNet, }

    # The following models currently support auxiliary heads
    aux_models = ['bisenetv2', 'ddrnet', 'icnet', 'cgfnet']

    # The following models currently support detail heads
    detail_head_models = ['stdc']

    # The following models currently support edge heads
    ...
    if config.model == 'smp':   # Use segmentation models pytorch
        ...
        # elif config.model in edge_head_models:
        #     model = model_hub[config.model](num_class=config.num_class, use_edge_head=config.use_edge_head, use_aux=config.use_aux)
        else:
            if config.use_aux:
