...
class SegTrainer(BaseTrainer):
    ...
    def train_one_epoch(self, config):
        ...
        for cur_itrs, (images, masks) in enumerate(pbar):
            ...
            elif config.use_edge_head:
                edges = edges.unsqueeze(1).float()
                with amp.autocast(enabled=config.amp_training):
                    preds, preds_aux = self.model(images, is_training=True) # b,c,h,w = [8,19,1024,1024]   [8,19,1024,1024]   [8,1,1024,1024]
                    size = masks.size()[1:] # 原图大小
                    preds_aux = F.interpolate(preds_aux, size, mode='bilinear', align_corners=True) # 辅助头上采样8倍
                    # preds_edge = F.interpolate(preds_edge, size, mode='bilinear', align_corners=True) # 边界上采样8倍
                    loss = self.loss_fn(preds, masks)  + 0.4 * self.loss_fn(preds_aux, masks) # 总损失函数
