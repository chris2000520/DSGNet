...
from utils import (get_optimizer, get_scheduler, parallel_model, de_parallel, 
                    ...
                    destroy_ddp_process, mkdir, save_config, log_config, matplot_loss, )
...
class BaseTrainer:
    ...
    def run(self, config):
        ...
        for cur_epoch in range(start_epoch, config.total_epoch):
            ...
            # 当前训练loss
            cur_train_loss = self.train_one_epoch(config)
            cur_train_loss = cur_train_loss.cpu()
            # cur_train_loss = cur_train_loss.detach().numpy()
            ...
            if cur_epoch >= config.begin_val_epoch and cur_epoch % config.val_interval == 0:
                val_score, cur_val_loss = self.validate(config)
                # cur_val_loss = cur_val_loss.cpu().detach().numpy()
            ...
            # 绘制图
            # self.logger.info(f'Train_loss:{train_loss}\n')
            # self.logger.info(f'Val_loss  :{val_loss}\n')
            # train_process = pd.DataFrame(data={"epoch":range(cur_epoch+1),
